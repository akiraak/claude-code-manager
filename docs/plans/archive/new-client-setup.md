...
### Phase 3: macOS 固有の確認 ✅ 実機検証済み (2026-06-19, darwin 25.5.0)
...
#### 実機検証ログ (Mac 単体・非破壊)
1. **python3 所在**: `command -v python3` → `/opt/homebrew/bin/python3` (Homebrew, 3.14.5)。
   `/usr/bin/python3` (CLT) も併存。setup スクリプトの絶対パス解決 (`command -v`) が機能することを確認。
2. **プロセス検出 (ps + lsof)**:
   - `ps -axww -o pid=,comm=,command=` が本物の `claude` プロセス 2 件 (comm=`claude`) を検出。
     コマンドラインに "claude" を含む zsh shell-snapshot 行は comm/argv0 が `zsh` のため正しく除外。
   - `lsof -a -d cwd -p <pids> -Fpn` が各 pid の cwd を取得。
   - 実体 `listClaudeProcesses()` (compiled dist) が `{pid, cwd}` 2 件を返すことをエンドツーエンドで確認
     (本セッション = `claude-code-manager` を含む)。
   - `processes.test.ts` 9 ケース全 pass (macOS 実行)。
3. **hook (実 python3 で全イベント)**: PermissionRequest→marker 生成 (schema = session_id/cwd/tool_name/
   tool_use_id/created_at) / PostToolUse→削除 / Stop(stop_hook_active=true)→no-op / Stop(normal)→削除 /
   不正 stdin→静かに exit 0、をすべて確認。テスト marker は後始末済み (残骸なし)。
4. **marker スキーマ整合**: reader `awaiting-input.ts` の `AWAITING_INPUT_DIR =
   '/tmp/claude-code-manager/awaiting-input'` と hook の `MARKER_DIR` が一致。reader が読む
   `session_id`/`tool_name`/`created_at` を hook が書き込むことを確認。

**結論**: macOS 固有の懸念 (python3 所在 / `/proc` 不在に伴う ps+lsof 経路 / パス) はすべて解消済み。
Phase 4 (ドキュメント反映) のみ残る。
