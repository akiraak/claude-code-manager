...
test('ルータ: onVoiceEvent は受理した voice-event でのみ検証済み payload で発火する', async () => {
  let clock = 0;
  const seen: string[] = [];
  await withServer(
    {
      ...
      now: () => clock,
      onVoiceEvent: (v) => { seen.push(v.kind); },
    },
    async (base) => {
      await post(base, '/api/ingest/voice-event', validVoiceBody({ kind: 'completed' }));
      // クールダウン中 (429) は発火しない
      await post(base, '/api/ingest/voice-event', validVoiceBody({ kind: 'completed' }));
      // 不正 body (400) は発火しない
      await post(base, '/api/ingest/voice-event', { clientId: 'c' });
      // snapshot では発火しない
      await post(base, '/api/ingest/snapshot', validSnapshotBody());
